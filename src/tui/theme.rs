...
impl Theme {
    pub fn load(name: &str) -> Self {
        match name {
            "omarchy" => Self::try_omarchy_dewey()
    ...
    fn try_omarchy_dewey() -> Option<Self> {
    ...
    fn try_omarchy_colors() -> Option<Self> {
        ...
        let bg = hex_to_color(&colors.background)?;
        let fg = hex_to_color(&colors.foreground)?;
        // color8 is "bright black" which is often too close to background.
        // Compute a visible muted color by blending bg→fg at ~50%.
        let muted = blend_colors(bg, fg, 0.50).unwrap_or(hex_to_color(&colors.color8)?);
        Some(Self {
            background: bg,
            foreground: fg,
    ...
    fn omarchy_theme_path() -> Option<PathBuf> {
        let home = std::env::var("HOME").ok()?;
        Some(PathBuf::from(home).join(".config/omarchy/current/theme/dewey.toml"))
    }
    ...
    /// Base style with explicit background — use on blocks, widgets, and areas.
    pub fn style_base(&self) -> Style {
        Style::default().bg(self.background).fg(self.foreground)
    }

    /// Text style (fg only) — use on spans inside list items so selection bg bleeds through.
...
fn blend_colors(c1: Color, c2: Color, factor: f32) -> Option<Color> {
    if let (Color::Rgb(r1, g1, b1), Color::Rgb(r2, g2, b2)) = (c1, c2) {
        Some(Color::Rgb(
            (r1 as f32 + factor * (r2 as f32 - r1 as f32)) as u8,
            (g1 as f32 + factor * (g2 as f32 - g1 as f32)) as u8,
            (b1 as f32 + factor * (b2 as f32 - b1 as f32)) as u8,
        ))
    } else {
        None
    }
