...
pub fn draw_task_list(f: &mut Frame, app: &App, theme: &Theme, area: Rect) {
    ...
    let legend = Line::from(vec![
        ...
        Span::styled(" nav  ", theme.style_muted()),
        Span::styled("⏎", theme.style_accent()),
    ...
    let status_bar = Paragraph::new(Text::from(vec![legend])).block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(theme.style_muted())
            .style(theme.style_base()),
    ...
    // Toast notification — top-right overlay, auto-dismisses
    if let Some((msg, level, _)) = &app.status_message {
        ...
        let msg_width = display_msg.len() as u16;
        // Clamp width to available space with some margin
        let toast_width = msg_width.min(task_area.width.saturating_sub(4));

        let toast_area = Rect {
            x: task_area.x + task_area.width.saturating_sub(toast_width + 2),
            y: task_area.y + 1,
            width: toast_width + 2,
            height: 3,
        };

        let toast = Paragraph::new(display_msg)
...
fn format_task_line<'a>(task: &'a Task, theme: &'a Theme, width: u16) -> Line<'a> {
    ...
    let source_label = format!("[{}]", task.backend_key);
...
pub fn draw_help(f: &mut Frame, theme: &Theme, area: Rect) {
    let help_text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled("     View task details", theme.style_default()),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            Span::styled("A", theme.style_accent()),
            Span::styled("         Launch AI agent on task", theme.style_default()),
        ]),
        Line::from(vec![
            Span::styled("S", theme.style_accent()),
            Span::styled("         Show running agents", theme.style_default()),
        ]),
        Line::from(vec![
            Span::styled("L", theme.style_accent()),
            Span::styled("         Setup wizard", theme.style_default()),
        ]),
        Line::from(vec![
            Span::styled("?", theme.style_accent()),
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            "Quick-add supports: #tags +project @backends (p1/p2/p3) today/tomorrow/YYYY-MM-DD",
            theme.style_muted(),
        )]),
    ];
    ...
    let area = super::centered_rect(60, 80, area);
