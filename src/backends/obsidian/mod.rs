...
impl ObsidianConfig {
    pub fn from_table(table: &toml::Table) -> Result<Self> {
        let vault_path = table
            ...
            .map(PathBuf::from)
            .ok_or_else(|| DeweyError::Config("obsidian.vault_path is required".into()))?;
...
impl ObsidianBackend {
    ...
    fn parse_file_tasks(&self, path: &Path) -> Result<Vec<Task>> {
        let content = fs::read_to_string(path).map_err(|e| DeweyError::Backend {
        ...
        let tasks = parsed
            ...
            .map(|(line_num, parsed)| Task {
                ...
                backend_key: "obsidian".to_string(),
        ...
        F: FnOnce(&str) -> String,
    {
        ...
        let idx = line_num.checked_sub(1).ok_or_else(|| DeweyError::Backend {
            backend: "obsidian".to_string(),
        ...
        fs::write(path, output).map_err(|e| DeweyError::Backend {
            backend: "obsidian".to_string(),
    ...
    fn parse_task_id(id: &TaskId) -> Result<(String, usize)> {
        let rest = id.strip_prefix("obsidian:").ok_or_else(|| {
            DeweyError::Parse(format!("Invalid Obsidian task ID: {}", id))
        })?;

        let last_colon = rest.rfind(':').ok_or_else(|| {
            DeweyError::Parse(format!("Invalid Obsidian task ID format: {}", id))
        })?;

        let rel_path = &rest[..last_colon];
        let line_num: usize = rest[last_colon + 1..]
            .parse()
            .map_err(|_| DeweyError::Parse(format!("Invalid line number in task ID: {}", id)))?;
...
impl TaskBackend for ObsidianBackend {
    ...
    async fn create_task(&self, task: &NewTask) -> Result<Task> {
        ...
        if !inbox_path.exists() {
            fs::write(&inbox_path, "").map_err(|e| DeweyError::Backend {
                backend: "obsidian".to_string(),
                message: format!("Failed to create inbox file: {}", e),
            })?;
        }

        let mut content = fs::read_to_string(&inbox_path).map_err(|e| DeweyError::Backend {
            backend: "obsidian".to_string(),
        ...
        fs::write(&inbox_path, &content).map_err(|e| DeweyError::Backend {
            backend: "obsidian".to_string(),
    ...
    async fn update_task(&self, id: &TaskId, update: &TaskUpdate) -> Result<Task> {
        ...
        let content =
            fs::read_to_string(&abs_path).map_err(|e| DeweyError::Backend {
                backend: "obsidian".to_string(),
    ...
    async fn delete_task(&self, id: &TaskId) -> Result<()> {
        ...
        fs::write(&abs_path, output).map_err(|e| DeweyError::Backend {
            backend: "obsidian".to_string(),
...
",
    ...
    async fn test_create_task() {
        ...
        let new_task = NewTask {
            title: "New task from dewey".to_string(),
