        ...
        if let Some(parent) = path.parent() {
            if !parent.exists() {
                std::fs::create_dir_all(parent).map_err(|e| {
                    DeweyError::Config(format!("Failed to create {}: {}", parent.display(), e))
                })?;
            }
        }
...
impl TaskBackend for LocalFileBackend {
    ...
    async fn update_task(&self, id: &TaskId, update: &TaskUpdate) -> Result<Task> {
        let line_num: usize = id
            .strip_prefix("local:")
            .ok_or_else(|| DeweyError::Parse(format!("Invalid task ID: {}", id)))?
            .parse()
            .map_err(|_| DeweyError::Parse(format!("Invalid task ID: {}", id)))?;

        if !self.config.path.exists() {
            return Err(DeweyError::Io(std::io::Error::new(
        ...
        if line_num == 0 || line_num > lines.len() {
            return Err(DeweyError::Parse(format!("Line {} not found", line_num)));
        }

        let current_line = &lines[line_num - 1];
        let mut task = self.parse_line(current_line, line_num)
            .ok_or_else(|| DeweyError::Parse(format!("Could not parse line {}", line_num)))?;
